...
cqueue2* initialize_cqueue2(int size) {
	...
	cq->bounds[2] = size;
    printf("Values are as folows:\n");
    printf("%d %d\n", cq->front[0], cq->front[1]);
    printf("%d %d\n", cq->rear[0], cq->rear[1]);
    printf("%d %d %d\n", cq->bounds[0], cq->bounds[1], cq->bounds[2]);

    return cq;
}

void insertcq(cqueue2* cq, int ele, int q) {
	if(cq->bounds[q-1]+(cq->rear[q-1]+1-cq->bounds[q-1])%(cq->bounds[q]-cq->bounds[q-1]) == cq->front[q-1]) {
		printf("Queue Full\n");
		return;
	}
	cq->rear[q-1] = cq->bounds[q-1] + (cq->rear[q-1] + 1 - cq->bounds[q-1]) % (cq->bounds[q] - cq->bounds[q-1]);
...
int deletecq(cqueue2* cq, int q) {
	...
	if(cq->front[q-1] == cq->rear[q-1]) 
		x =  -5555;
	else if(cq->bounds[q-1] + (cq->front[q-1]+1-cq->bounds[q-1])%(cq->bounds[q]-cq->bounds[q-1]) == cq->rear[q-1]) {
		x = cq->arr[cq->rear[q-1]];
		cq->front[q-1] = cq->rear[q-1] = cq->bounds[q-1];
	}
	else {
		cq->front[q-1] = cq->bounds[q-1] + (cq->front[q-1]+1-cq->bounds[q-1]) % (cq->bounds[q]-cq->bounds[q-1]);
...
void displaycq(cqueue2* cq, int q) {
	...
	while(i != cq->rear[q-1]) {
		printf("\t%d", cq->arr[i]);
		i = (i >= cq->bounds[q]) ? cq->bounds[q-1]:i+1;
	}
